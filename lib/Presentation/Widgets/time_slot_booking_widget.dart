...
class DoctorBooking extends StatefulWidget {
  ...
  final String date;
  final String fee;
  final List<DoctorsModel>? availableTime;
  final int doctorId;
  final int index;
...
class _DoctorBookingState extends State<DoctorBooking> {
  ...
  void initState() {
    ...
    if (widget.availableTime!.isNotEmpty) {
      timeByDate = widget
          ...
          .where((element) =>
              element.date!.split('T').first.contains(widget.date))
          .toList();
            ...
            builder: (context) => Dialog(
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    const Text("Appoinment SucceccFully completed"),
                    TextButton(
            ...
            builder: (context) => Dialog(
              child: Container(
                ...
                child: Column(
                  ...
                  mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                  children: [
                    const Text("Failed To get Appoinments"),
                    TextButton(
                      onPressed: () => Navigator.of(context).pop(),
                      child: const Text('OK'),
                    ),
                                    ...
                                    return GestureDetector(
                                      onTap: () {
                                        ...
                                        selectedTime = times[index]
                                            ...
                                            .split('.')
                                            .first;
                                        timeSlotId = times[index].id!.toInt();
                                      },
                                      child: Container(
                                        height: mHeight * .05,
                                        width: mWidth * .30,
  ...
  void handlerPaymentSuccess() {
    ...
    BlocProvider.of<AppoinmentsBloc>(context).add(
      TakeAppoinmentsEvent(timeSlotId, reasonController.text),
    );
